...
SELECT 	
        CASE WHEN oc.customer_gender ='M' THEN 'MR'
			 WHEN oc.customer_gender = 'F' THEN 'MS'
		END AS TITLE ,
             UPPER(CONCAT( oc.customer_fname, ' ', oc.customer_lname)) AS fullname,
		oc.customer_email, oc.customer_creation_date,
CASE
	WHEN  YEAR(oc.customer_creation_date) < 2005
			THEN 'Category A'
	WHEN  YEAR(oc.customer_creation_date) >= 2005 AND YEAR(oc.customer_creation_date) < 2011
			THEN 'Category B'
	WHEN  YEAR(oc.customer_creation_date) >= 2011
			THEN 'Category C'
...
SELECT 	
		oi.product_id, p.product_id, p.product_desc,p.product_quantity_avail,p.product_price, (p.product_price * p.product_quantity_avail) as inventory_values,
CASE
	WHEN   p.product_price > 200000
			THEN '20% discount'
	WHEN   p.product_price > 100000
			THEN '15% discount'
	WHEN   p.product_price <= 100000
			THEN '10% discount'
	END AS 'CUSTOMER DISCOUNT'
FROM product p, order_items oi;
		
        
desc product_class;

desc product_class_code;
...
select oc.customer_id,CONCAT( oc.customer_fname, ' ', oc.customer_lname) AS fullname,sum(oi.product_quantity),
(oi.product_quantity * p.product_price ) as total_value from online_customer oc, order_items oi, product p,
 order_header oh where oh.order_status = 'Shipped' and oc.customer_lname like 'G%' and oh.payment_mode = 'Cash'; 

/*9*/

SELECT 
p.product_id,p.product_desc,sum(oi.product_quantity)
from  product p, order_items oi 
...
/*10*/
SELECT 
	oi.order_id, oc.customer_id,
    UPPER(CONCAT( oc.customer_fname, ' ', oc.customer_lname)) AS fullname,
    oi.product_quantity from order_items oi, online_customer oc, address a  
